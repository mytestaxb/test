...
def main(args):
    ...
    # writer=SummaryWriter(logdir='./model_graph/',comment=curr_run)
...
if __name__ == "__main__":
    if len(sys.argv[1:])==0:
        argv=['--use_valuenorm', '--use_popart', '--env_name', 'MPE', '--algorithm_name', 'rmappo',
              ...
              '--num_landmarks', '3', '--seed', '1', '--n_training_threads', '10', '--n_rollout_threads',
              '10', '--num_mini_batch', '1', '--episode_length', '25','--num_env_steps', '20000000',
              '--ppo_epoch', '10', '--use_ReLU', '--gain', '0.01', '--lr', '7e-4', '--critic_lr',
              '7e-4', '--wandb_name', 'zoeyuchao', '--user_name', 'zoeyuchao','--use_wandb','False',
              'cuda','False', 'cuda_deterministic','False']
    else:
        argv=sys.argv[1:]
    main(argv)
